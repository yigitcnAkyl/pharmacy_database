...
1. **`01_create_tables.sql`**: Veritabanı tablolarının, Primary Key (Birincil Anahtar) ve Foreign Key (Yabancı Anahtar) ilişkilerinin tanımlandığı şema kurulum dosyası.
2. **`02_insert_data.sql`**: İlişkisel bütünlük (Integrity Constraints) sıralaması gözetilerek veritabanına eklenen örnek veri setleri.
